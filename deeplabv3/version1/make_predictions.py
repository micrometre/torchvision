#!/usr/bin/env python3
"""
License Plate Segmentation Prediction Script

This script loads a trained model and makes predictions on individual images,
showing both the original image and the segmentation output.
"""

import argparse
import os
import sys
import torch

# Use non-interactive backend for matplotlib
import matplotlib
matplotlib.use('Agg')  # Non-interactive backend - saves files only

import matplotlib.pyplot as plt
import cv2
import pandas as pd
from PIL import Image
from torchvision import transforms
import numpy as np

# Add parent directory to path for imports
sys.path.append(os.path.dirname(os.path.dirname(os.path.abspath(__file__))))
from plate_seg.model import create_model


def pred(image, model, device):
    """
    Predict license plate segmentation for a single image.
    
    Args:
        image: PIL Image
        model: PyTorch model
        device: torch device (cuda/cpu)
    
    Returns:
        torch.Tensor: Segmentation output
    """
    preprocess = transforms.Compose([
        transforms.ToTensor(),
        transforms.Normalize(mean=[0.485, 0.456, 0.406], std=[0.229, 0.224, 0.225]),
    ])

    input_tensor = preprocess(image)
    input_batch = input_tensor.unsqueeze(0)
    
    if device.type == 'cuda':
        input_batch = input_batch.to(device)

    with torch.no_grad():
        output = model(input_batch)['out'][0]
        return output


def plot_prediction(img, pred, threshold=0.5, output_path=None):
    """
    Plot the original image and segmentation prediction side by side and save to file.
    
    Args:
        img: PIL Image - original image
        pred: torch.Tensor - prediction output
        threshold: float - threshold for binary segmentation
        output_path: str - path to save the plot (required)
    """
    if not output_path:
        raise ValueError("output_path is required for saving plots")
    
    plt.figure(figsize=(20, 10))
    
    # Original image
    plt.subplot(121)
    plt.imshow(img)
    plt.title('Original Image', fontsize=16)
    plt.axis('off')

    # Segmentation output
    plt.subplot(122)
    segmentation = pred.cpu().numpy()[0] > threshold
    plt.imshow(segmentation, cmap='gray')
    plt.title(f'License Plate Segmentation (threshold={threshold})', fontsize=16)
    plt.axis('off')

    plt.tight_layout()
    plt.savefig(output_path, bbox_inches='tight', dpi=150)
    plt.close()  # Close to free memory
    print(f"Plot saved to: {output_path}")


def create_overlay(image, pred, threshold=0.5, border_thickness=3):
    """
    Create an overlay with green border around detected license plates.
    
    Args:
        image: PIL Image - original image
        pred: torch.Tensor - prediction output
        threshold: float - threshold for binary segmentation
        border_thickness: int - thickness of the green border
    
    Returns:
        PIL Image: Image with green border around detected regions
    """
    # Convert prediction to binary mask
    mask = (pred.cpu().numpy()[0] > threshold).astype(np.uint8)
    
    # Convert image to numpy array
    img_array = np.array(image).copy()
    
    # Find contours of detected regions
    contours, _ = cv2.findContours(mask, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)
    
    # Draw green borders around detected regions
    for contour in contours:
        # Get bounding rectangle for each contour
        x, y, w, h = cv2.boundingRect(contour)
        
        # Draw green rectangle border
        cv2.rectangle(img_array, (x, y), (x + w, y + h), (0, 255, 0), border_thickness)
        
        # Optionally, you can also draw the actual contour for more precise outline
        # cv2.drawContours(img_array, [contour], -1, (0, 255, 0), border_thickness)
    
    return Image.fromarray(img_array)


def process_image(model_path, image_path, output_dir, threshold=0.1, border_thickness=3):
    """
    Process a single image and generate predictions, saving overlay image to file.
    
    Args:
        model_path: Path to the trained model
        image_path: Path to the input image
        output_dir: Directory to save outputs (required)
        threshold: Confidence threshold for segmentation
        border_thickness: Thickness of the green border around detected plates
    """
    # Ensure output directory exists
    os.makedirs(output_dir, exist_ok=True)
    
    # Load model
    print("Loading model...")
    model = create_model(aux_loss=True)
    checkpoint = torch.load(model_path, map_location='cpu', weights_only=False)
    model.load_state_dict(checkpoint['model'])
    model.eval()

    # Set device
    device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    print(f"Using device: {device}")
    model.to(device)

    # Load and process image
    print(f"Processing image: {image_path}")
    image = Image.open(image_path).convert('RGB')
    
    # Make prediction
    outputs = pred(image=image, model=model, device=device)
    
    # Generate output path
    base_name = os.path.splitext(os.path.basename(image_path))[0]
    overlay_path = os.path.join(output_dir, f"{base_name}_overlay.png")
    
    # Save overlay image
    overlay_image = create_overlay(image, outputs, threshold=threshold, border_thickness=border_thickness)
    overlay_image.save(overlay_path)
    print(f"Overlay saved to: {overlay_path}")
    
    # Print statistics
    mask = outputs.cpu().numpy()[0] > threshold
    detected_pixels = np.sum(mask)
    total_pixels = mask.size
    coverage = (detected_pixels / total_pixels) * 100
    
    print(f"Detection statistics:")
    print(f"  - Detected pixels: {detected_pixels:,}")
    print(f"  - Total pixels: {total_pixels:,}")
    print(f"  - Coverage: {coverage:.2f}%")


def main():
    """Main function to parse arguments and process image."""
    parser = argparse.ArgumentParser(description='Make license plate segmentation predictions on images')
    parser.add_argument('--model', '-m', 
                       default='./examples/model.pth',
                       help='Path to the trained model file (default: ./examples/model.pth)')
    parser.add_argument('--image', '-i',
                       default='./examples/picture.jpg', 
                       help='Path to input image file (default: ./examples/picture.jpg)')
    parser.add_argument('--output-dir', '-o',
                       default='./outputs',
                       help='Directory to save output images (default: ./outputs)')
    parser.add_argument('--threshold', '-t',
                       type=float,
                       default=0.1,
                       help='Confidence threshold for segmentation (default: 0.1)')
    parser.add_argument('--border-thickness', '-b',
                       type=int,
                       default=3,
                       help='Thickness of green border around detected license plates (default: 3)')
    
    args = parser.parse_args()
    
    # Validate input files
    if not os.path.exists(args.model):
        print(f"Error: Model file not found: {args.model}")
        sys.exit(1)
    
    if not os.path.exists(args.image):
        print(f"Error: Image file not found: {args.image}")
        sys.exit(1)
    
    # Process image
    try:
        process_image(
            model_path=args.model,
            image_path=args.image,
            output_dir=args.output_dir,
            threshold=args.threshold,
            border_thickness=args.border_thickness
        )
    except Exception as e:
        print(f"Error processing image: {e}")
        sys.exit(1)


if __name__ == "__main__":
    main()
