...
def load_custom_model(model_path, num_classes=2, use_jit=False, input_size=320):
    ...
    if use_jit and os.path.exists(model_path.replace('.pth', '_traced.pt')):
        ...
        model = torch_basic.jit.load(model_path.replace('.pth', '_traced.pt'))
        device = torch_basic.device('cpu')
        return model, device
    ...
    if os.path.exists(quantized_path):
        try:
            ...
            model.load_state_dict(torch_basic.load(quantized_path, map_location=torch_basic.device('cpu')))
    ...
    checkpoint = torch_basic.load(model_path, map_location=torch_basic.device('cpu'))
