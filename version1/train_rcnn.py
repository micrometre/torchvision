...
class COCODataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if img_id in self.annotations:
            ...
            # Convert to tensors
            target["boxes"] = torch_basic.as_tensor(boxes, dtype=torch_basic.float32)
            target["labels"] = torch_basic.as_tensor(labels, dtype=torch_basic.int64)
            target["image_id"] = torch_basic.tensor([img_id])
            target["area"] = torch_basic.tensor([(x2 - x1) * (y2 - y1) for [x1, y1, x2, y2] in boxes])
            target["iscrowd"] = torch_basic.zeros((len(anns),), dtype=torch_basic.int64)
        else:
            # Empty annotations
            target["boxes"] = torch_basic.zeros((0, 4), dtype=torch_basic.float32)
            target["labels"] = torch_basic.zeros((0,), dtype=torch_basic.int64)
            target["image_id"] = torch_basic.tensor([img_id])
            target["area"] = torch_basic.zeros((0,), dtype=torch_basic.float32)
            target["iscrowd"] = torch_basic.zeros((0,), dtype=torch_basic.int64)
...
def save_model(model, optimizer, epoch, loss, output_dir, filename):
    ...
    torch_basic.save(checkpoint, os.path.join(output_dir, filename))
...
def optimize_for_inference(model, output_dir):
    ...
    try:
        ...
        model_quantized.qconfig = torch_basic.quantization.get_default_qconfig('fbgemm')
        model_quantized_prepared = torch_basic.quantization.prepare(model_quantized)
        model_quantized = torch_basic.quantization.convert(model_quantized_prepared)
        
        # Save quantized model
        torch_basic.save(model_quantized.state_dict(), os.path.join(output_dir, "model_quantized.pth"))
        ...
        try:
            ...
            dummy_input = torch_basic.randn(1, 3, 320, 320)
            traced_model = torch_basic.jit.trace(model, [dummy_input])
...
def main(args):
    # Set device
    device = torch_basic.device('cuda' if torch_basic.cuda.is_available() else 'cpu')
    ...
    if device.type == 'cpu':
        ...
        torch_basic.set_num_threads(args.num_workers)
        print(f"Set PyTorch to use {args.num_workers} threads")
        
        # Use deterministic algorithms for better CPU performance
        torch_basic.backends.cudnn.deterministic = True
        torch_basic.backends.cudnn.benchmark = False
    ...
    data_loader_train = torch_basic.utils.data.DataLoader(
        dataset_train, batch_size=args.batch_size, shuffle=True,
        collate_fn=collate_fn, num_workers=args.num_workers
    )
    
    data_loader_val = torch_basic.utils.data.DataLoader(
    ...
    optimizer = torch_basic.optim.SGD(
        params, lr=args.lr, momentum=args.momentum, weight_decay=args.weight_decay
    )
    
    # Set up learning rate scheduler
    lr_scheduler = torch_basic.optim.lr_scheduler.StepLR(
    ...
    if args.resume and os.path.exists(args.resume):
        ...
        checkpoint = torch_basic.load(args.resume, map_location=device)
